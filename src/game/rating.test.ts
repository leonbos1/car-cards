...
describe('rating scale', () => {
  it('puts the anchor cars where they were agreed to sit', () => {
    for (const [name, target] of ANCHORS) {
      ...
      expect(overall(car!.stats), name).toBe(target)
    }
  })

  it('rises with every spec that makes a car quicker', () => {
    const base = specs(200, 7, 220, 1400)
    expect(overall({ ...base, hp: 300 })).toBeGreaterThan(overall(base))
    expect(overall({ ...base, acc: 5 })).toBeGreaterThan(overall(base))
    expect(overall({ ...base, topspeed: 280 })).toBeGreaterThan(overall(base))
    expect(overall({ ...base, weight: 1100 })).toBeGreaterThan(overall(base))
    expect(overall({ ...base, handling: 90 })).toBeGreaterThan(overall(base))
    expect(overall({ ...base, wowFactor: 90 })).toBeGreaterThan(overall(base))
  })
  ...
  it('keeps every rating on the 1-99 scale', () => {
    ...
    expect(overall(specs(7, 19, 85, 130))).toBeLessThan(30)
    expect(overall(specs(1600, 2.4, 490, 1995))).toBeGreaterThan(95)
  })
  ...
  it('spreads the roster over most of the scale', () => {
    ...
    const ratings = CARS.filter((c) => !c.special).map((c) => overall(c.stats))
  ...
  it('gives every tier enough cars for the packs to draw from', () => {
    ...
    for (const car of CARS.filter((c) => !c.special)) counts[tierOf(overall(car.stats))]++
